...
class Param(object):
    ...
    def get_index(self):
        return min(
            i
            for i in range(len(self._parent_obj._params))
            if self == self._parent_obj._params[i]
        )
        ...
        for node in NodeRep.parse(node):
            ...
            if type(node) is phpast.Variable:
                ...
                if type(node._parent_node) is phpast.ObjectProperty:
                    varname = f'{node._parent_node.node.name}->{node._parent_node.name}' 
...
#                 fc = FuncCall(node.name, node.lineno, node, scope, self)
                ...
                if called_obj := fc.get_called_obj():
